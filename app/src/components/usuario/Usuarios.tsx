...
export default function Usuarios() {
  ...
  useEffect(() => {
    async function getUsuarios() {
      try {
        const data = await fetchWithAuth("http://localhost:3000/api/usuarios");
  ...
  const handleExcluir = async (cpf: string) => {
    if (confirm("Tem certeza que deseja excluir este usuário?")) {
      try {
        await fetchWithAuth(`http://localhost:3000/api/usuarios/${cpf}`, { method: "DELETE" });
                ...
                {usuarios.map((usuario) => (
                  <tr key={usuario.cpf} className="border-b dark:border-gray-700">
                    ...
                    <td className="py-2 px-4 border flex space-x-2">
                      <button onClick={() => handleEditar(usuario.cpf)} className="btn-editar">
                        Editar
                      </button>
                      <button onClick={() => handleExcluir(usuario.cpf)} className="btn-delete">
                        Excluir
                      </button>
                    </td>
                  </tr>
