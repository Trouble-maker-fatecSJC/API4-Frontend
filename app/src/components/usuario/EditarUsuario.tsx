...
export default function EditarUsuario() {
    ...
    useEffect(() => {
        console.log(cpfEdicao);
        if (!cpfEdicao) return; // Se cpfEdicao não estiver definido, não faz nada

        async function fetchUsuario() {
            try {
                const response = await fetch(`http://localhost:3000/api/usuarios/${cpfEdicao}`);
                if (response.ok) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch(`http://localhost:3000/api/usuarios/${cpf}`, {
                method: 'PUT',
